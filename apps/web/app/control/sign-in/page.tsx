...
async function signIn(formData: FormData) {
  ...
  if (!outcome.ok) redirect(`/control/sign-in?error=${outcome.reason}`);
...
async function signInAsDemo(formData: FormData) {
  ...
  if (email !== DEMO_OPERATOR_ACCOUNT.email) redirect('/control/sign-in?error=invalid_credentials');
...
export default async function SignInPage({
  ...
  if (await currentOperator()) redirect('/control');
