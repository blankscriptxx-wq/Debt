 ...
 *
 * Three deliberate choices in this component. Accept is not the primary action -
 ...
 * Rejecting requires a reason, so the pattern can be reviewed later. And a
 * suggestion touching regulated information is visually distinct, so it never
 * looks like ordinary housekeeping.
 */
export function ProposalDecision({ caseId, proposal }: { caseId: string; proposal: ProposalView }) {
  ...
  function decide(decision: 'accepted' | 'modified' | 'rejected') {
    ...
    startTransition(async () => {
      const response = await fetch(`/api/cases/${caseId}/proposals/${proposal.id}`, {
        ...
        body: JSON.stringify({
          decision,
          appliedValue: decision === 'modified' ? coerce(value, proposal.proposedValue) : undefined,
          note: note || undefined,
        }),
      });
      if (!response.ok) {
  ...
  return (
    <div className={`sv-proposal${proposal.touchesRegulatedField ? ' sv-proposal--regulated' : ''}`}>
      ...
      <p style={{ margin: '8px 0 0', color: 'var(--ink-muted)' }}>{proposal.reasoning}</p>
      ...
      {mode === 'modify' && (
        <div>
          <label className="sv-field">
            ...
            <input className="sv-input" value={value} onChange={(e) => setValue(e.target.value)} />
          </label>
          <label className="sv-field">
